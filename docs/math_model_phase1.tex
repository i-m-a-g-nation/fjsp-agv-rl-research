...
\section{Experiment Result Schema}

Phase 1 experiment scripts write normalized CSV files under
\texttt{experiments/results/}. Each row has the following fields:
...
Field & Meaning \\
\midrule
\texttt{experiment\_id} & stable experiment identifier \\
\texttt{instance\_name} & benchmark or toy instance name \\
\texttt{algorithm} & solver or dispatching rule name \\
\texttt{makespan} & objective value, empty if no schedule was returned \\
\texttt{feasible} & \texttt{PASS} or \texttt{FAIL} after feasibility checking \\
\texttt{runtime\_s} & measured wall-clock runtime in seconds \\
\texttt{note} & solver status, error, or validation note \\
\bottomrule
\end{tabular}
\end{table}

The CSV layer is intentionally simple. It records what happened in each
experiment without changing solver behavior or feasibility definitions.
